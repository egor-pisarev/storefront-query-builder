...
var RequestBody = /** @class */ (function () {
    function RequestBody(_a) {
        ...
        this.baseFilters = {
            range: {
                check: function (_a) {
                    var value = _a.value;
                    return Object.keys(value).every(function (v) { return _this_1.rangeOperators.includes(v); });
                },
                filter: function (_a) {
            ...
            orRange: {
                check: function (_a) {
                    var value = _a.value;
                    return Object.keys(value).every(function (o) { return _this_1.orRangeOperators.includes(o); });
                },
                filter: function (_a) {
                    var attribute = _a.attribute, value = _a.value, queryChain = _a.queryChain;
                    for (var o in value) {
        ...
        this.catalogFilterBuilder = function (filterQr, filter, attrPostfix, type) {
            ...
            if (_this_1.checkIfObjectHasScope({ object: filter, scope: 'catalog' }) && valueKeys.length > 0) {
                var isRange = valueKeys.filter(function (value) { return _this_1.rangeOperators.indexOf(value) !== -1; });
                if (isRange.length) {
                    var rangeAttribute = attribute;
                    // filter by product fiunal price
                    if (rangeAttribute === 'price') {
                        rangeAttribute = _this_1.config.products.priceFilterKey;
                    }
                    ...
                    if (attrPostfix === '') {
                        filterQr = filterQr[type]('terms', _this_1.getMapping(attribute), newValue);
                    }
                    else {
         ...
         * @return {any}
         */
        this.bodybuilder = function () { return _this_1.emptyQueryChain.clone(); };
        this.config = config;
    ...
    RequestBody.prototype.applyBaseFilters = function () {
        ...
        this.appliedFilters.forEach(function (filter) {
            var value = filter.value, attribute = filter.attribute;
            if (!_this_1.checkIfObjectHasScope({ object: filter, scope: 'default' })) {
                return;
            }
            value = typeof value === 'object' ? value : { 'in': [value] };
            if (Object.keys(value).length > 0) {
                attribute = _this_1.getMapping(attribute);
                var operator = Object.keys(value)[0];
                var sortedFilters = _this_1.getSortedFilters();
                for (var _i = 0, sortedFilters_1 = sortedFilters; _i < sortedFilters_1.length; _i++) {
                    var filterHandler = sortedFilters_1[_i];
                    var queryChain = _this_1.queryChain;
                    ...
                    if (filterHandler.check({ operator: operator, attribute: attribute, value: value, queryChain: queryChain })) {
                        ...
                        _this_1.queryChain = filterHandler.filter.call(_this_1, { operator: operator, attribute: attribute, value: value, queryChain: queryChain });
                        return;
                    }
                }
                // add filter attribute that is not handled by sortedFilters list
                _this_1.queryChain = _this_1.queryChain.filter('terms', attribute, _this_1.extractFirstValueMutator(value));
            }
        });
        return this;
    };
    ...
    RequestBody.prototype.applyCatalogFilters = function () {
        ...
        var catalogFilters = this.appliedFilters.filter(function (object) {
        ...
        this.appliedFilters.forEach(function (filter) {
            var _a = filter.attribute.split('.'), path = _a[0], attribute = _a[1];
            if (attribute) {
        ...
        rootFilters.forEach(function (filter) {
            _this_1.queryChain.filter('bool', function (catalogFilterQuery) {
                return _this_1.catalogFilterBuilder(catalogFilterQuery, filter, undefined, 'orFilter')
                    .orFilter('bool', function (b) { return _this_1.catalogFilterBuilder(b, filter, _this_1.optionsPrefix).filter('match', 'type_id', 'configurable'); });
            });
        });
        Object.keys(nestedFilters).forEach(function (path) {
            _this.queryChain.query('nested', { path: path }, function (q) {
                catalogFilters.forEach(function (filter) {
                    if (filter.value.in) {
                        q.query('terms', filter.attribute, filter.value.in);
                    }
                    else if (filter.value.eq) {
                        q.query('term', filter.attribute, filter.value.eq);
                    }
                });
                return q;
            });
    ...
    RequestBody.prototype.hasCatalogFilters = function () {
        ...
        if (!this._hasCatalogFilters) {
            this._hasCatalogFilters = this.searchQuery.getAppliedFilters()
                .some(function (object) { return _this_1.checkIfObjectHasScope({ object: object, scope: 'catalog' }); });
        }
